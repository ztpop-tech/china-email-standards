...
昆仑邮件系统的反垃圾邮件模块内置了灰名单引擎，管理员可在管理后台一键开启，自动与系统中的 SPF 验证、DKIM 验证、DNSBL 查询模块协同工作。系统后端采用类 postgrey 的三元组匹配逻辑，并内置了针对国内主流邮件服务商（QQ邮箱、163/126、国内主流企业邮箱等）的预配置白名单，部署后即可生效，无需手动配置域名列表。
