...
*本文由 [ztpop.net 知识库](https://www.ztpop.net/kb/stalwart-mail-server-deployment-guide.html) 镜像发布。采用 [CC-BY 4.0](https://creativecommons.org/licenses/by/4.0/) 许可，可自由引用，仅需标注来源。*

---

发布于 2026-08-03
    Stalwart v0.16.x（Rust，AGPL-3.0，13.9k+ GitHub stars）集 SMTP/IMAP4rev2/JMAP/POP3/CalDAV/CardDAV 于单一二进制，内置 DMARC/DKIM2/DMARCbis/ARC/Sieve/AI 垃圾过滤，无需 Postfix+Dovecot+OpenDKIM 多组件拼装。本文基于官方文档与 GitHub 仓库，提供从零到生产级的完整部署路径：二进制脚本安装、Docker 部署、Bootstrap 引导、DNS 配置、认证体系内置配置、存储后端选型，以及从 Postfix+Dovecot 迁移的步骤对照。

本文是《新一代开源邮件基础设施盘点：KumoMTA、Stalwart 与 mail-auth（2026）》的配套实操篇，面向有意将 Stalwart 投入生产部署的运维工程师。Stalwart 最新稳定版为 v0.16.16（2026-08-02 发布），是当前首个同时支持 DKIM2（draft-ietf-dkim-dkim2-spec）与 DMARCbis（RFC 9989/9990/9991）的开源邮件服务器实现[1]。

## 一、系统要求与前置条件

Stalwart 由 Rust 编写，原生支持 Linux、macOS、FreeBSD 与 Windows。根据官方文档[2]：
...
CPU 需求与并发量正相关；x86_64 与 ARM64（鲲鹏/飞腾等信创平台）均支持，Rust 的跨架构编译能力使其在信创硬件上具备天然优势。
...
如需使用 FoundationDB 作为存储后端（高可用集群场景），安装时加 --fdb 参数[1]：
...
Docker 适合快速验证或隔离部署。官方镜像为 stalwartlabs/stalwart:v0.16[1]：
...
访问 http://:8080/admin，使用 Bootstrap 凭证登录后，Web UI 提供向导式配置：

1. 管理员账户：创建永久管理员（替换临时 admin）
1. 域名配置：添加邮件域名，设置 MX 记录要求
1. 存储后端：选择 RocksDB（默认）/ PostgreSQL / MySQL / SQLite / FoundationDB / S3 等
1. 证书配置：ACME 自动申请（推荐）或手动上传 PEM
1. 服务端口：确认 SMTP/IMAP/JMAP 等协议监听端口
...
Stalwart 内置 ACME 自动申请 Let's Encrypt 证书，但 DNS 记录需手动配置[3]：
...
Stalwart 支持 stalwart-cli 命令行工具查看自动生成的 DNS 记录[1]：

```
$ stalwart-cli query domain
$ stalwart-cli get domain  --fields dnsZoneFile
```

## 七、内置邮件认证体系配置

Stalwart v0.16.x 是 2026 年对邮件认证新标准支持最完整的开源实现[4]。所有认证功能在 Web UI 中一键启用，无需外接 OpenDKIM/OpenDMARC。

### 7.1 DKIM2 签名（draft-ietf-dkim-dkim2-spec）

Stalwart 是首个在生产代码中实现 DKIM2 监管链签名机制的邮件服务器。DKIM2 新增特性[5]：
...
Stalwart 自动处理 ARC 链：转发邮件时自动插入 ARC-Set 与 ARC-Message-Signature 头部；接收时验证 ARC 验签并保留 cv=pass 链，使 DMARC 校验失败邮件（如转发导致的 From 对齐问题）仍可被放行[6]。使用 ARC 校验器可诊断转发链完整性。

### 7.4 SPF / DANE / MTA-STS

Stalwart SMTP 入站验证内建 SPF（RFC 7208）校验，支持 DANE（RFC 6698）TLSA 记录与 MTA-STS（RFC 8461）强制 TLS 策略。使用 SPF 深度诊断工具验证配置合规性。

## 八、存储后端选型

Stalwart 存储分为三层：数据存储（账户/域/配置）、Blob 存储（邮件正文/附件）、搜索索引[2]。各层均支持多种后端：
...
Stalwart 可替代「Postfix（SMTP）+ Dovecot（IMAP/POP3）+ OpenDKIM（签名）+ OpenDMARC（验证）+ Rspamd（过滤）」多组件栈，提供单一二进制统一管理。以下迁移步骤[7]供参考：

1. 评估阶段：在测试环境安装 Stalwart，确认协议兼容性（IMAP4rev2/rev1、JMAP、Sieve 均已支持）；使用 域名健康评分工具评估当前域名认证状态
1. DNS 预配置：在 DNS 添加 Stalwart 的 MX 记录，但主 MX 仍指向旧服务器；配置 SPF/DKIM/DMARC/MTA-STS 记录指向新服务器 IP（并行运行期）
1. 用户迁移：导出 Dovecot 邮件存储（Maildir/mbox）到 Stalwart 支持的格式；导入账户数据（Stalwart 支持 SQL 目录 / LDAP / 内置数据库）
1. 切换 MX：修改 DNS 将 MX 主记录切换到 Stalwart；旧服务器保留作为备份 MX（优先级 20）
1. 验证阶段：使用 SPF 诊断、DMARC 报告解析器、parsedmarc 自托管教程验证认证体系运行正常
1. 下线旧组件：确认投递无误后，下线 Postfix/Dovecot/OpenDKIM/OpenDMARC/Rspamd

Stalwart 官方提供从 Dovecot、 Cyrus、Exchange 的迁移指南[7]，建议在正式迁移前完整阅读。

## 十、信创环境注意事项

Stalwart 在信创环境（鲲鹏/飞腾/海光等 ARM64 平台、麒麟/统信 OS）部署时需关注以下事项：

- Rust 跨平台编译：Stalwart 提供 Linux x86_64 与 ARM64 预编译二进制，信创平台建议从源码编译（cargo build --release --target aarch64-unknown-linux-gnu）以获得最佳性能
- AGPL-3.0 许可：内部自用与 SaaS 提供不受传染；商业闭源产品集成需评估许可证约束[4]
- 国密算法：Stalwart 当前版本（v0.16.x）不含 SM2/SM3/SM4 国密支持；信创合规场景需自行评估 GB/T 37002-2026[8]的要求缺口
- 依赖审计：Rust crate 依赖链需纳入供应链安全评估

## 十一、常见问题

### Q1：Bootstrap 模式密码丢失怎么办？
进入恢复模式（Recovery Mode）重新生成管理员凭证，详见官方恢复模式文档[1]。
...
v0.16.x 引入声明式配置（YAML/JSON）替代旧版 TOML，数据库 Schema 有变更；从 v0.15.x 升级需阅读官方升级文档[1]。

## 十二、相关工具

Stalwart 生产部署中，推荐配合以下工具验证配置状态：

- SPF 深度诊断工具：验证 SPF 记录语法、lookup 次数、ptr 误用
- DMARC XML 报告解析器：解析 Stalwart 发出的 RUA 聚合报告
- ARC 校验器：验证 Stalwart 转发生成的 ARC 链
- 域名健康评分：8 项聚合检查（SPF/DKIM/DMARC/MTA-STS/BIMI/DNSBL 等）
- parsedmarc 自托管教程：DMARC/TLS-RPT 报告解析开源方案

### 参考文献
1. stalwartlabs/stalwart — GitHub 官方仓库（AGPL-3.0，Rust），v0.16.16 发布于 2026-08-02，13,982 stars
1. Stalwart System Requirements — 官方文档，stalw.art
1. Stalwart Setting up DNS — 官方文档，stalw.art
1. 新一代开源邮件基础设施盘点：KumoMTA、Stalwart 与 mail-auth（2026） — ztpop.net 知识库
1. DKIM2 监管链签名机制深度解读 — ztpop.net 知识库
1. DMARCbis RFC 9989 概览：DMARC 2.0 的核心变更 — ztpop.net 知识库
1. Stalwart Migrating from another system — 官方迁移指南，stalw.art
1. GB/T 37002-2026《网络安全技术 电子邮件系统安全技术规范》解读 — ztpop.net 知识库
...
1. RFC 8617 — Authenticated Received Chain (ARC) Protocol
1. RFC 8461 — SMTP MTA Strict Transport Security (MTA-STS)
1. RFC 8460 — SMTP TLS Reporting
1. RFC 8463 — Ed25519 and Ed448 Public Keys in DKIM
1. RFC 9989 — DMARCbis Core Specification (2026)
1. RFC 9990 — DMARCbis Aggregate Reporting (2026)
1. RFC 9991 — DMARCbis Failure Reporting (2026)

  ### 引用本文
  ztpop.net 邮件技术知识库编辑. "Stalwart 邮件服务器部署实操指南：从安装到生产级配置（2026）." ztpop.net 邮件技术知识库.

  本站技术文章采用 CC-BY 4.0 许可，可自由引用，仅需标注来源 ztpop.net。

  发布者声明：本文基于 Stalwart 官方 GitHub 仓库（v0.16.16）与官方文档（stalw.art）编写，仅供技术参考。

  CC-BY 4.0 许可，转载请注明出处。原文链接：https://www.ztpop.net/kb/stalwart-mail-server-deployment-guide.html

---

*原文链接：https://www.ztpop.net/kb/stalwart-mail-server-deployment-guide.html | ztpop.net 邮件技术知识库*
