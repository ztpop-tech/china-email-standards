...
早期 SMTP 默认允许"开放中继"（任何人都可借你的服务器转发第三方邮件），被垃圾邮件放大利用。如今所有正规 MTA 都默认仅允许已认证用户或本地网络发信。开放中继既是声誉灾难，也是法律与合规风险。信创[邮件系统部署](/kb/category/ops-architecture.html)时，必须把"禁止开放中继 + 强制发信认证"作为出厂基线。
