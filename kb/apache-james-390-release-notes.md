...
*本文由 [ztpop.net 知识库](https://www.ztpop.net/kb/apache-james-390-release-notes.html) 镜像发布。采用 [CC-BY 4.0](https://creativecommons.org/licenses/by/4.0/) 许可，可自由引用，仅需标注来源。*

---

发布于 2026-08-03
    Apache James（Java Apache Mail Enterprise Server）是 Apache 软件基金会旗下的 JVM 邮件服务器，支持 SMTP/LMTP/POP3/IMAP/ManageSieve/JMAP 全套协议。3.9.0 是 James 历史上变化最大的一次发布：运行时从 Java 11 迁移到 Java 21、命名空间从 javax 迁移到 jakarta、新增 JAMES-2586 高性能 Postgres 实现，并完成多项协议现代化与安全加固。本文基于官方 CHANGELOG（james-project-3.9.0 tag）逐项解读。

Apache James 以模块化 Mailet 容器架构著称，允许通过组件拼装定制邮件处理流水线。3.9.0 版本（GitHub: apache/james-project，1037 stars / 489 forks）的发布标志着项目进入现代 JVM 时代。以下解读基于官方 CHANGELOG 文件（james-project-3.9.0 tag，2025 年发布）[1]。
...
JAMES-3954 实现 RFC 9394 PARTIAL 扩展，为 IMAP FETCH/SEARCH 提供分页语义。RFC 9394 定义了基于位置的范围检索（PARTIAL 命令），客户端可在不加载整个邮箱的情况下分页获取邮件元数据，显著降低大邮箱场景的内存与带宽消耗。
...
1. 依赖替换：自定义 Mailet 中 javax.* → jakarta.* 全量替换
1. 配置迁移：CassandraModule 更名影响 Guice 装配代码；检查 Cassandra*DataDefinition 新名称
1. 存储评估：新部署可评估 Postgres 实现（JAMES-2586）替代 Cassandra
1. JMAP 客户端：确认客户端兼容 RFC 8621 正式版（draft 已移除）
1. 回归测试：重点验证 Mailet 流水线、SMTP 认证、IMAP 同步行为

## 七、与 2026 年开源邮件生态的定位

James 3.9.0 与 Rust 系新项目（Stalwart 部署实操指南）形成差异化：James 面向 JVM 生态与 Mailet 可编程流水线，适合需要深度定制邮件处理逻辑的 Java 团队；Stalwart 以单二进制与内置认证见长。两者与 KumoMTA（高性能外发）共同构成 2026 年开源邮件基础设施的三大路线，详细对照见《新一代开源邮件基础设施盘点》。

在协议层面，James 的 JMAP 支持（RFC 8620/8621）与 IMAP4rev2（RFC 9051）对齐现代客户端趋势，相关协议背景可参考本站《JMAP 邮件访问协议深度解析》与《IMAP vs POP3 vs JMAP 选型决策》。
...
### 参考文献
1. Apache James Project CHANGELOG — 官方变更日志（james-project-3.9.0 tag）
1. Apache James Server 3 Release Notes — 官方发布说明
1. RFC 9394 — IMAP PARTIAL Extension for Listing and Search
1. RFC 8621 — The JSON Meta Application Protocol (JMAP) for Mail
1. RFC 9051 — IMAP4rev2
1. RFC 5321 — Simple Mail Transfer Protocol
1. RFC 6376 — DomainKeys Identified Mail (DKIM) Signatures
1. RFC 3207 — SMTP Service Extension for Secure SMTP over TLS
1. apache/james-project — GitHub 官方仓库（1037 stars / 489 forks）

  ### 引用本文
  ztpop.net 邮件技术知识库编辑. "Apache James 3.9.0 新特性解读：Java 21 迁移与 Postgres 高性能实现（2026）." ztpop.net 邮件技术知识库.

  本站技术文章采用 CC-BY 4.0 许可，可自由引用，仅需标注来源 ztpop.net。

  发布者声明：本文基于 Apache James 官方 CHANGELOG（james-project-3.9.0）与官方发布说明编写，仅供技术参考。

  CC-BY 4.0 许可，转载请注明出处。原文链接：https://www.ztpop.net/kb/apache-james-390-release-notes.html

---

*原文链接：https://www.ztpop.net/kb/apache-james-390-release-notes.html | ztpop.net 邮件技术知识库*
