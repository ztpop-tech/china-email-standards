...
| --- | --- | --- |
| 国内主流企业邮箱 / QQ 邮箱 | 内部信誉评分 + 内容过滤 + 用户投诉反馈回路 | 腾讯内部可能部署了自己的 Spamtrap 网络，但数据不对外开放 |
| 阿里企业邮箱 / 国内主流企业邮箱件推送 | 内部信誉 + 规则引擎 + AI 内容分析 | 阿里云推送有独立的反垃圾团队，自建检测系统 |
| 国内主流企业邮箱 / 163 邮箱 | 内部过滤引擎 + DKIM/SPF 验证 + 行为模式分析 | 强大的自研过滤系统，Spamtrap 是内部工具之一 |
| 国内主流邮箱 | 内部规则 + 外部黑名单有限引用 | 引用部分国际 DNSBL 作为辅助 |
