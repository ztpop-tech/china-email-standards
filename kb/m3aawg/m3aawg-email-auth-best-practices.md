...
截至 2026 年，国内大型邮件服务商（如 QQ 邮箱、163 邮箱、国内主流企业邮箱）对 DMARC 策略的处理存在较大差异。部分服务商对 `p=reject` 的执行力度较弱，甚至对未通过 DMARC 认证的邮件仍以 `p=none` 逻辑投递。这意味着在国内环境下，仅靠 `p=reject` 可能无法实现真正的拒绝效果。
