...
国内主流邮箱（QQ 邮箱、163 邮箱、国内主流邮箱等）提供的"自动转发"功能在转发邮件时，往往会对邮件头部进行修改（例如插入自己的 `Received` 头部、改写 `From` 或 `Subject` 内容标识），甚至在某些场景下会重新封装邮件体。这些操作会破坏原发信域的 DKIM 签名，导致接收端的 DMARC 验证失败（尤其是 `p=reject` 策略的发信域），最终造成邮件被拒收或进入垃圾箱。此问题在国内非常普遍，影响了大量使用邮箱转发功能的用户，尤其是企业员工将公司邮件转发至个人邮箱的场景。
