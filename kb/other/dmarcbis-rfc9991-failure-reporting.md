...
### 6.3 国产邮件系统（国产邮件系统 等）的适配现状

国产邮件系统 作为国内市场份额最大的邮件系统，在 DMARC 支持方面已具备基础能力，但针对 RFC 9991 的适配仍需关注以下方向：

* **故障报告生成**：当前 国产邮件系统 等国产邮件系统在 DMARC 验证失败时，多数采用内部日志记录方式而非 IODEF + ARF 标准格式输出。后续建设需要对接国际标准交换格式
